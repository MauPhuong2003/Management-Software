...
import MiniGame, { IWheelSlot } from '../models/MiniGame';
...
function weightedRandom(slots: any[]): number {
...
// ──────────────────────────────────────────────
export const getMiniGame = async (req: Request, res: Response): Promise<void> => {
    try {
        const minigame = await MiniGame.findOne().populate('slots.giftId');
        if (minigame) {
            ...
            if (filteredSlots.length !== originalLength) {
                ...
                await MiniGame.updateOne(
                    { _id: minigame._id },
                    { $set: { slots: filteredSlots.map(s => ({ giftId: (s.giftId as any)._id || s.giftId, color: s.color, probability: s.probability })) } }
                );
            }
        }
...
// ──────────────────────────────────────────────
export const upsertMiniGame = async (req: Request, res: Response): Promise<void> => {
    try {
        const body = req.body;

        // Validate slot probabilities if slots are provided
        if (body.slots && Array.isArray(body.slots) && body.slots.length > 0) {
            const total = (body.slots as IWheelSlot[]).reduce((sum, s) => sum + Number(s.probability || 0), 0);
        ...
        if (!minigame) {
            minigame = await MiniGame.create(body);
        } else {
            Object.assign(minigame, body);
            await minigame.save();
        }

        const populated = await MiniGame.findById(minigame._id).populate('slots.giftId');
...
// ──────────────────────────────────────────────
export const toggleMiniGame = async (req: Request, res: Response): Promise<void> => {
    try {
...
// ──────────────────────────────────────────────
export const getSpinHistory = async (req: Request, res: Response): Promise<void> => {
    try {
        const page = Math.max(1, parseInt(req.query.page as string) || 1);
        const limit = Math.min(100, parseInt(req.query.limit as string) || 20);
        const skip = (page - 1) * limit;
        const prizeType = req.query.prizeType as string | undefined;
...
export const updateRewardStatus = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const record = await SpinHistory.findById(historyId);
        if (!record) {
            res.status(404).json({ success: false, message: 'Không tìm thấy lịch sử quay' });
            return;
        }

        if (rewardStatus) record.rewardStatus = rewardStatus;
...
// ──────────────────────────────────────────────
export const resetSlotQuantity = async (req: Request, res: Response): Promise<void> => {
    try {
...
// ──────────────────────────────────────────────
export const getActiveMiniGame = async (req: CustomerAuthRequest, res: Response): Promise<void> => {
    try {
        const minigame = await MiniGame.findOne({ isActive: true }).populate({
            path: 'slots.giftId',
        ...
        const safeSlots = minigame.slots
            .filter((s: any) => s.giftId !== null)
            .map((s: any) => {
                const gift = s.giftId as any;
                const voucher = gift?.voucherId as any;
        ...
        if (req.customer) {
            ...
            if (minigame.maxSpinsPerDay > 0) {
                ...
                spinsToday = await SpinHistory.countDocuments({
...
// ──────────────────────────────────────────────
export const exchangePointsForSpins = async (req: CustomerAuthRequest, res: Response): Promise<void> => {
    try {
        ...
        const minigame = await MiniGame.findOne({ isActive: true });
        if (!minigame) {
...
// ──────────────────────────────────────────────
export const spinWheel = async (req: CustomerAuthRequest, res: Response): Promise<void> => {
    try {
        const customer = req.customer;
        if (!customer) {
            res.status(401).json({ success: false, message: 'Chưa đăng nhập' });
        ...
        let selectedSlot = minigame.slots[slotIndex] as any;
        let gift = selectedSlot.giftId as IGift | null;
        ...
        if (gift && gift.prizeType !== 'no_prize' && gift.quantityRemaining <= 0) {
            ...
            const fallbackIndex = minigame.slots.findIndex((s: any) => s.giftId?.prizeType === 'no_prize');
            if (fallbackIndex !== -1) {
                slotIndex = fallbackIndex;
                selectedSlot = minigame.slots[fallbackIndex];
                gift = selectedSlot.giftId as any;
                isFallback = true;
            }
        }
        ...
        let generatedVoucherCode: string | undefined = undefined;

        if (gift && gift.voucherId) {
            let templatePromo: any = gift.voucherId;
            if (typeof templatePromo !== 'object' || !templatePromo.code) {
                templatePromo = await Promotion.findById(gift.voucherId);
            }
            ...
            if (templatePromo) {
                ...
                const userVoucher = await Promotion.create({
        ...
        if (gift && gift.prizeType !== 'no_prize') {
            await Gift.updateOne(
                { _id: gift._id },
                { $inc: { quantityRemaining: -1, claimedCount: 1 } }
            );
        }
        ...
        const finalVoucherCode = generatedVoucherCode || (gift?.voucherId && (typeof gift.voucherId === 'object') ? (gift.voucherId as any).code : undefined);
...
// ──────────────────────────────────────────────
export const getCustomerSpinHistory = async (req: CustomerAuthRequest, res: Response): Promise<void> => {
    try {
        const customer = req.customer;
        if (!customer) {
            res.status(401).json({ success: false, message: 'Chưa đăng nhập' });
        ...
        const [records, total] = await Promise.all([
            SpinHistory.find({ customerId: customer._id })
                .populate('giftId')
                .sort({ spinAt: -1 })
                .skip(skip)
                .limit(limit),
            SpinHistory.countDocuments({ customerId: customer._id })
        ]);
